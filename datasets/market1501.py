...
class Market1501(dataset.Dataset):
    def __init__(self, args, transform, dtype):
        ...
        camera_set = set()
        for path in list_pictures(data_path):
            camera_set.add(self.camera(path))
        self.camera_num = len(camera_set)
    ...
    def id(file_path):
        ...
        :return: person id
        """
        return int(file_path.split(os.sep)[-1].split('_')[0])
    ...
    def camera(file_path):
        ...
        :return: camera id
        """
        return int(file_path.split(os.sep)[-1].split('_')[1][1:3])
